              ...
              {features.length > 0 && (
                <div style={{ marginBottom: '2.5rem' }}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
                    {features.map((f, i) => (
                      <span key={i} style={{ fontFamily: 'var(--sans)', fontSize: '0.72rem', letterSpacing: '0.15em', textTransform: 'uppercase', color: '#fff', border: '1px solid #2A2A2A', padding: '0.4rem 0.85rem' }}>{f}</span>
                    ))}
                  </div>
                </div>
              )}
