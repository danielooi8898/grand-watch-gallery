...
import { Save, Loader2, Plus, Trash2, CheckCircle, Eye } from 'lucide-react'
    ...
    supabase.from('site_settings').select('value').eq('key', key).single()
      .then(({ data }) => {
        if (data?.value !== undefined && data.value !== null) {
          setVal(typeof data.value === 'string' ? (() => { try { return JSON.parse(data.value) } catch { return data.value } })() : data.value)
        }
      })
  }, [key])
    ...
    <div style={{ padding:'2rem 2rem 4rem', maxWidth:'900px' }}>
      ...
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:'2rem', paddingBottom:'1.5rem', borderBottom:'1px solid #EDE9E3', flexWrap:'wrap', gap:'1rem' }}>
        ...
        <a href="/" target="_blank" rel="noopener noreferrer"
          style={{ display:'inline-flex', alignItems:'center', gap:'0.4rem', padding:'0.5rem 1rem', border:'1px solid #EDE9E3', borderRadius:'4px', background:'#fff', textDecoration:'none', fontFamily:'var(--sans)', fontSize:'0.72rem', color:'#666', transition:'border-color 0.15s' }}
      ...
      {activeTab === 'Hero' && (
        <div style={card}>
          ...
          <div style={{ display:'grid', gap:'1rem' }}>
            <Field label="Small Label (above headline)" hint='Shown above the main headline in gold. e.g. "EST. 2020 · AUTHENTICATED TIMEPIECES"'>
            ...
            <Field label="Headline" hint='Use a new line (Enter) to split into 2 lines. The first word of line 2 shows in gold.'>
              <textarea style={{ ...inp, minHeight:'72px', resize:'vertical' }}
                value={hero.title || ''}
                onChange={e => setHero(p => ({ ...p, title: e.target.value }))}
                onFocus={e => e.target.style.borderColor='#B08D57'} onBlur={e => e.target.style.borderColor='#E0DDD8'} />
            </Field>
            <Field label="Description Paragraph" hint='Short intro text below the headline. Use a new line to create a line break.'>
      ...
      {activeTab === 'Stats' && (
        <div style={card}>
          <div style={{ marginBottom:'1.25rem' }}>
            ...
            <p style={{ fontFamily:'var(--sans)', fontSize:'0.75rem', color:'#999' }}>The three numbers shown below the hero buttons and in the stats section.</p>
          </div>
          <div style={{ display:'grid', gap:'1rem' }}>
            {(stats || []).map((s, i) => (
              <div key={i} style={{ display:'grid', gridTemplateColumns:'1fr 80px 2fr', gap:'0.75rem', alignItems:'end', padding:'1rem', background:'#F7F6F3', borderRadius:'6px' }}>
                <Field label={`Stat ${i+1} — Number`}>
                  <input style={inp} value={s.n} placeholder="500"
                    onChange={e => { const a=[...stats]; a[i]={...a[i],n:e.target.value}; setStats(a) }}
                    onFocus={e => e.target.style.borderColor='#B08D57'} onBlur={e => e.target.style.borderColor='#E0DDD8'} />
                </Field>
                <Field label="Suffix">
                  <input style={inp} value={s.s} placeholder="+"
                    onChange={e => { const a=[...stats]; a[i]={...a[i],s:e.target.value}; setStats(a) }}
                    onFocus={e => e.target.style.borderColor='#B08D57'} onBlur={e => e.target.style.borderColor='#E0DDD8'} />
                </Field>
                <Field label="Label">
                  <input style={inp} value={s.l} placeholder="Watches Sold"
                    onChange={e => { const a=[...stats]; a[i]={...a[i],l:e.target.value}; setStats(a) }}
                    onFocus={e => e.target.style.borderColor='#B08D57'} onBlur={e => e.target.style.borderColor='#E0DDD8'} />
                </Field>
              </div>
            ))}
            ...
            {(services || []).map((s, i) => (
              <div key={i} style={{ padding:'1.25rem', background:'#F7F6F3', borderRadius:'6px', border:'1px solid #EDE9E3' }}>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'80px 1fr', gap:'0.75rem', marginBottom:'0.75rem' }}>
                  <Field label="Number">
                    <input style={inp} value={s.num}
                      onChange={e => { const a=[...services]; a[i]={...a[i],num:e.target.value}; setServices(a) }}
                      onFocus={e => e.target.style.borderColor='#B08D57'} onBlur={e => e.target.style.borderColor='#E0DDD8'} />
                  </Field>
                  <Field label="Title">
                    <input style={inp} value={s.title}
                      onChange={e => { const a=[...services]; a[i]={...a[i],title:e.target.value}; setServices(a) }}
                      onFocus={e => e.target.style.borderColor='#B08D57'} onBlur={e => e.target.style.borderColor='#E0DDD8'} />
                  </Field>
                </div>
                <Field label="Body Text">
                  <textarea style={{ ...inp, minHeight:'80px', resize:'vertical' }} value={s.body}
                    onChange={e => { const a=[...services]; a[i]={...a[i],body:e.target.value}; setServices(a) }}
                    onFocus={e => e.target.style.borderColor='#B08D57'} onBlur={e => e.target.style.borderColor='#E0DDD8'} />
                </Field>
              </div>
            ))}
      ...
      {activeTab === 'Testimonials' && (
        <div style={card}>
            ...
            {(testimonials || []).map((t, i) => (
              <div key={i} style={{ padding:'1.25rem', background:'#F7F6F3', borderRadius:'6px', border:'1px solid #EDE9E3' }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'0.75rem' }}>
                  ...
                  <button onClick={() => setTestimonials(testimonials.filter((_,j)=>j!==i))}
                    style={{ padding:'0.3rem 0.6rem', border:'1px solid #FCA5A5', background:'#FFF5F5', cursor:'pointer', color:'#DC2626', borderRadius:'4px', display:'flex', alignItems:'center', gap:'0.3rem', fontFamily:'var(--sans)', fontSize:'0.68rem' }}>
                    <Trash2 size={11}/> Remove
                  </button>
                </div>
                <div style={{ display:'grid', gap:'0.75rem' }}>
                  <Field label="Client Name">
                    <input style={inp} value={t.name} placeholder="e.g. Dato Ahmad R."
                      onChange={e => { const a=[...testimonials]; a[i]={...a[i],name:e.target.value}; setTestimonials(a) }}
                      onFocus={e => e.target.style.borderColor='#B08D57'} onBlur={e => e.target.style.borderColor='#E0DDD8'} />
                  </Field>
                  <Field label="Quote">
                    <textarea style={{ ...inp, minHeight:'80px', resize:'vertical' }} value={t.text} placeholder="Their quote here…"
                      onChange={e => { const a=[...testimonials]; a[i]={...a[i],text:e.target.value}; setTestimonials(a) }}
                      onFocus={e => e.target.style.borderColor='#B08D57'} onBlur={e => e.target.style.borderColor='#E0DDD8'} />
                  </Field>
                </div>
              </div>
          ...
          <div style={{ display:'flex', gap:'0.75rem', marginTop:'1rem' }}>
            <button onClick={() => setTestimonials([...(testimonials||[]),{name:'',text:''}])}
              style={{ display:'inline-flex', alignItems:'center', gap:'0.4rem', padding:'0.6rem 1rem', border:'1px solid #EDE9E3', background:'#fff', cursor:'pointer', fontFamily:'var(--sans)', fontSize:'0.72rem', color:'#555', borderRadius:'4px', transition:'border-color 0.15s' }}
              onMouseEnter={e => e.currentTarget.style.borderColor='#B08D57'}
              onMouseLeave={e => e.currentTarget.style.borderColor='#EDE9E3'}>
      ...
      {activeTab === 'Brands' && (
        <div style={card}>
          ...
          <Field label="Brand List (one per line)">
            <textarea style={{ ...inp, minHeight:'200px', resize:'vertical', fontFamily:'monospace', fontSize:'0.8rem', lineHeight:1.7 }}
              value={(brands||[]).join('\n')}
              onChange={e => setBrands(e.target.value.split('\n').map(s=>s.trim()).filter(Boolean))}
              onFocus={e => e.target.style.borderColor='#B08D57'} onBlur={e => e.target.style.borderColor='#E0DDD8'} />
          </Field>
          <p style={hint}>{(brands||[]).length} brands configured</p>
          <SaveBar saving={saving} onSave={() => doSave(saveBrands)} />
        </div>
      )}
    </div>
  )
