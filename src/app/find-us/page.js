      ...
      <section style={{ background:'#0A0A0A', paddingTop:'8rem', paddingBottom:'5rem', minHeight:'55vh' }}>
        <div className="container">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24">
            ...
            {/* Left */}
            <div>
              <p style={{ fontFamily:'var(--sans)', fontSize:'0.72rem', letterSpacing:'0.35em', textTransform:'uppercase', color:'#B08D57', marginBottom:'1.5rem' }}>Our Location</p>
              ...
              <div style={{ marginBottom:'3rem' }}>
                {addressLines.map((l,i) => (
                  <p key={i} style={{ fontFamily:'var(--sans)', fontSize:'1.05rem', color:'#fff', lineHeight:2.2, fontWeight:300 }}>{l}</p>
                ))}
              </div>

              <div style={{ marginBottom:'2.5rem' }}>
                <Link href="/appointment" style={{ display:'inline-flex', alignItems:'center', gap:'0.6rem', fontFamily:'var(--sans)', fontSize:'0.75rem', fontWeight:700, letterSpacing:'0.25em', textTransform:'uppercase', color:'#fff', textDecoration:'none', paddingBottom:'0.4rem', borderBottom:'1px solid rgba(255,255,255,0.2)' }}>
            ...
            <div>
              <div style={{ display:'flex', alignItems:'center', gap:'0.75rem', marginBottom:'2rem' }}>
                ...
                <p style={{ fontFamily:'var(--sans)', fontSize:'0.72rem', letterSpacing:'0.35em', textTransform:'uppercase', color:'#B08D57' }}>Opening Hours</p>
              </div>
              <div>
                {hourLines.map((line, i) => {
                  ...
                  return (
                    <div key={i} style={{ display:'flex', justifyContent:'space-between', alignItems:'baseline', padding:'1rem 0', borderBottom:'1px solid #1A1A1A' }}>
                      <span style={{ fontFamily:'var(--sans)', fontSize:'0.9rem', color:'#fff', fontWeight:400 }}>{day.trim()}</span>
                      <span style={{ fontFamily:'var(--sans)', fontSize:'0.85rem', color:'#fff', fontWeight:300 }}>{time || line}</span>
                    </div>
                  )
                })}
              </div>
        ...
        <div className="container">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <Link href="/appointment" style={{ display:'inline-flex', alignItems:'center', gap:'0.75rem', background:'#B08D57', color:'#fff', padding:'0.9rem 2rem', fontFamily:'var(--sans)', fontSize:'0.7rem', fontWeight:700, letterSpacing:'0.2em', textTransform:'uppercase', textDecoration:'none' }}>
